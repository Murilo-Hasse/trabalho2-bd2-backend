...
class ProdutoList(Resource, mixins.ListMixin):
    query = select((p.codigo, p.descricao, p.valor, p.quantidade, p.imagem,
                   p.fornecedor.nome, p.grupo.descricao) for p in models.Produto)
    dict = dicts.Produto
    ...
    def post(self):
        ...
        if len(args['imagem']) >= 1:
            with db_session:
                ...
                img_link = utils.decode_and_upload_to_dropbox(
                    b64_img_str,
                    f"""{args['descricao']} - {select(fornecedor.nome for fornecedor in models.Pessoa
                                                    if fornecedor.codigo == args['fornecedor']).first()}""",
                    args['extensao_imagem']
                )
